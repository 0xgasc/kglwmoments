...
class UMOCache {
  constructor() {
    ...
    this.cacheFile = path.join(this.cacheDir, 'kinggizzard-cache.json');
    this.cache = null;
  ...
  async loadCache() {
    try {
      ...
      console.log(`✅ King Gizzard cache loaded: ${this.cache.performances.length} performances, last updated: ${this.cache.lastUpdated}`);
      return this.cache;
    } catch (err) {
  ...
  async saveCache(cacheData) {
    try {
      ...
      console.log(`💾 King Gizzard cache saved: ${cacheData.performances.length} performances`);
    } catch (err) {
  ...
  async checkForNewShows(apiBaseUrl, currentCount) {
    try {
      console.log('🔍 Quick check for new King Gizzard shows...');
      
      // DIRECT API CALL - not through proxy
      const response = await fetch(`https://api.setlist.fm/rest/1.0/artist/f58384a4-2ad2-4f24-89c5-c7b74ae1cce7/setlists?p=1`, {
        headers: { 
          Accept: 'application/json',
          'x-api-key': process.env.SETLIST_FM_API_KEY,
          'User-Agent': 'KingGizzardArchive/1.0'
        },
  ...
  async buildFreshCache(apiBaseUrl, progressCallback) {
    ...
    try {
      console.log('🏗️ Building fresh King Gizzard cache with medley processing...');
      ...
      while (hasMore && page <= 300 && consecutiveEmptyPages < 15 && consecutiveErrors < 5) {
        try {
          ...
          // DIRECT API CALL - not through proxy
          const response = await fetch(`https://api.setlist.fm/rest/1.0/artist/f58384a4-2ad2-4f24-89c5-c7b74ae1cce7/setlists?p=${page}`, {
            headers: { 
              Accept: 'application/json',
              'x-api-key': apiKey,
              'User-Agent': 'KingGizzardArchive/1.0'
          ...
          if (!response.ok) {
            if (response.status === 429) {
              ...
              const retryResponse = await fetch(`https://api.setlist.fm/rest/1.0/artist/f58384a4-2ad2-4f24-89c5-c7b74ae1cce7/setlists?p=${page}`, {
                headers: { 
                  Accept: 'application/json',
                  'x-api-key': process.env.SETLIST_FM_API_KEY,
                  'User-Agent': 'KingGizzardArchive/1.0'
